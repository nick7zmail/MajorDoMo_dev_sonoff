   ...
   for($i=0;$i<$total;$i++) {
	foreach($res_online as $id_online){
		...
		if($res[$i]['ID']==$id_online['DEVICE_ID'] && ($id_online['TITLE']=='rssi' || $id_online['TITLE']=='gsm_rssi')) {
			if ($id_online['VALUE']>= -50) {$res[$i]['RSSI_LVL']=100; $res[$i]['RSSI_COLOR']='#0ef523';} 
			elseif ($id_online['VALUE']>= -70) {$res[$i]['RSSI_LVL']=77; $res[$i]['RSSI_COLOR']='#0cae1b';} 
			elseif ($id_online['VALUE']>= -85) {$res[$i]['RSSI_LVL']=52; $res[$i]['RSSI_COLOR']='#f0ad4e';} 
			elseif ($id_online['VALUE']>= -110) {$res[$i]['RSSI_LVL']=26; $res[$i]['RSSI_COLOR']='#d9534f';} 
			else {$res[$i]['RSSI_LVL']=0; $res[$i]['RSSI_COLOR']='#d90600';}
		}
	}
